...
import {AtomInteract} from './atom-interact';
...
describe('AtomInteract', () => {
  describe('disabled states', () => {
    it('hard disabled: sets disabled attribute on native elements', async () => {
      await render(`<button atomInteract disabled></button>`, {imports: [AtomInteract]});
    ...
    it('hard disabled: sets aria-disabled on non-native elements', async () => {
      await render(`<span atomInteract role="button" disabled></span>`, {imports: [AtomInteract]});
    ...
    it('soft disabled (disabledInteractive): removes native disabled and adds aria-disabled', async () => {
      await render(`<button atomInteract disabled disabledInteractive></button>`, {
        imports: [AtomInteract],
      });
      const button = screen.getByRole('button');
    ...
    it('soft disabled: non-native element gets aria-disabled and remains focusable', async () => {
      await render(`<span atomInteract role="button" disabled disabledInteractive></span>`, {
        imports: [AtomInteract],
      });
      const el = screen.getByRole('button');
      expect(el).not.toHaveAttribute('disabled');
    ...
    it('not disabled: no disabled attributes', async () => {
      await render(`<button atomInteract></button>`, {imports: [AtomInteract]});
      const button = screen.getByRole('button');
      expect(button).not.toHaveAttribute('disabled');
  ...
  describe('tabIndex', () => {
    ...
    it('respects custom tabIndex', async () => {
      await render(`<button atomInteract [tabIndex]="3"></button>`, {imports: [AtomInteract]});
    ...
    it('preserves custom tabIndex when soft disabled', async () => {
      await render(
        `<span atomInteract role="button" tabindex="5" disabled disabledInteractive></span>`,
        {imports: [AtomInteract]},
      );
  ...
  describe('keyboard events — soft disabled', () => {
    it('blocks Enter key', async () => {
      const {fixture} = await render(
        `<span atomInteract role="button" disabled disabledInteractive></span>`,
        {imports: [AtomInteract]},
      );
  ...
  describe('click blocking', () => {
    it('blocks click when hard disabled', async () => {
      ...
      await render(`<div atomInteract role="button" disabled (click)="onClick()"></div>`, {
        imports: [AtomInteract],
    ...
    it('blocks click when soft disabled', async () => {
      const clickSpy = vi.fn();
      await render(
        `<div atomInteract role="button" disabled disabledInteractive (click)="onClick()"></div>`,
        {imports: [AtomInteract], componentProperties: {onClick: clickSpy}},
      );
    ...
    it('allows click when not disabled', async () => {
      const clickSpy = vi.fn();
      await render(`<div atomInteract role="button" tabindex="0" (click)="onClick()"></div>`, {
        imports: [AtomInteract],
        componentProperties: {onClick: clickSpy},
      });
  ...
  describe('focus management', () => {
    ...
    it('hard disabled non-native is not focusable via Tab', async () => {
      await render(`<span atomInteract role="button" disabled></span>`, {
        imports: [AtomInteract],
      });
      await userEvent.keyboard('[Tab]');
      expect(screen.getByRole('button')).not.toHaveFocus();
  ...
  describe('state transitions', () => {
    it('should update attributes when disabled changes dynamically', async () => {
      const {rerender, fixture} = await render(
        `<button atomInteract [disabled]="isDisabled"></button>`,
        {imports: [AtomInteract], componentProperties: {isDisabled: false}},
      );
    ...
    it('should switch between hard and soft disable', async () => {
      const {rerender, fixture} = await render(
        `<button atomInteract [disabled]="true" [disabledInteractive]="isInteractive"></button>`,
        {imports: [AtomInteract], componentProperties: {isInteractive: false}},
      );
  ...
  describe('element types', () => {
    it('should use native disabled on input elements', async () => {
      await render(`<input atomInteract disabled type="text" />`, {imports: [AtomInteract]});
    ...
    it('should use native disabled on select elements', async () => {
      await render(`<select atomInteract disabled><option>A</option></select>`, {
        imports: [AtomInteract],
      });
    ...
    it('should use aria-disabled on anchor elements', async () => {
      await render(`<a atomInteract disabled href="#">Link</a>`, {imports: [AtomInteract]});
    ...
    it('should use aria-disabled on div elements', async () => {
      await render(`<div atomInteract disabled role="button">Action</div>`, {
        imports: [AtomInteract],
      });
      const el = screen.getByRole('button');
      expect(el).toHaveAttribute('aria-disabled', 'true');
  ...
  describe('loading state use case', () => {
    it('should maintain focusability during loading transitions', async () => {
      const {rerender, fixture} = await render(
        `<button atomInteract [disabled]="isLoading" [disabledInteractive]="isLoading">Submit</button>`,
        {imports: [AtomInteract], componentProperties: {isLoading: false}},
      );
