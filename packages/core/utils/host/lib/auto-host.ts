...
export function autoHost<T>(
  name: string,
  factory: (ctx: ContextRef) => T,
): (injector?: Injector) => T {
  @AutoHost()
  class HostValue {
    readonly value = setupContext().runInContext((ctx) => factory(ctx));
  }
  ...
  return (injector?: Injector): T => {
    return setupContext(injector, autoHost).runInContext(() => inject(HostValue).value as T);
  };
}
