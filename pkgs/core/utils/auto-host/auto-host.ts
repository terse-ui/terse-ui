...
import {createInjCtxRef, injCtx, injectElement, type InjCtxRef} from '@terse-ui/core/internal';
import type {Constructor} from '@terse-ui/core/types';
...
export class AutoHostResolver {
  ...
  resolve<T extends object>(type: Constructor<T>, injector?: Injector): T {
    return injCtx(this.resolve.bind(this), injector, ({injector}) => {
...
export function autoHost<T>(
  ...
  @AutoHost()
  class HostValue {
    readonly value = factory(createInjCtxRef());
  }

  Object.defineProperty(HostValue, 'name', {value: name});
  Object.defineProperty(HostValue.prototype, Symbol.toStringTag, {value: name});

  return function injectHostValue(injector?: Injector): T {
    return injCtx(injectHostValue, injector, () => inject(HostValue).value as T);
  };
}
